...
// Vendor payment distribution return type
type VendorPaymentDistributionResult = {
  totalAmount: string;
...
async getPaymentsByInvoice(tenantId: string, invoiceId: string): Promise<PaymentWithDetails[]> {
  ...
  async createPayment(tenantId: string, paymentData: InsertPayment): Promise<PaymentWithDetails> {
    ...
    if (isNaN(paymentAmount) || paymentAmount <= 0) {
      throw new ValidationError('Invalid payment amount', { amount: 'Payment amount must be positive' });
    }
    ...
    if (['Bank', 'UPI', 'Cheque'].includes(paymentData.paymentMode) && !paymentData.bankAccountId) {
      throw new ValidationError('bankAccountId is required for non-cash payments', {
        bankAccountId: 'Bank account is required for Bank, UPI, or Cheque payments'
      });
    }
    ...
    try {
      return await db.transaction(async (tx) => {
        ...
        const appliedAmount = Math.min(paymentAmount, totalAmount - currentPaidAmount);
        ...
        if (newBalanceAmount <= epsilon) newStatus = 'Paid';
        else if (newPaidAmount > epsilon) newStatus = 'Partially Paid';
        ...
        if (payment.paymentMode === 'Cash') {
          const lastCashEntry = await tx.select().from(cashbook)
            ...
            .orderBy(desc(cashbook.id)).limit(1).for('update');

          const currentBalance = lastCashEntry.length ? parseFloat(lastCashEntry[0].balance) : 0;
          ...
          const lastBankEntry = await tx.select().from(bankbook)
            ...
            .orderBy(desc(bankbook.date), desc(bankbook.id)).limit(1).for('update');

          const currentBalance = lastBankEntry.length ? parseFloat(lastBankEntry[0].balance) : 0;
          const newBalance = currentBalance - appliedAmount;
        ...
        return {
          ...payment,
          invoice: { ...invoice, paidAmount: newPaidAmount.toFixed(2), balanceAmount: newBalanceAmount.toFixed(2), status: newStatus },
          vendor,
          bankAccount
        };
      });
    ...
    try {
      return await db.transaction(async (tx) => {
        const vendor = await tx.select().from(vendors)
          ...
          .limit(1);
        if (!vendor.length) throw new NotFoundError('Vendor');
        ...
        const outstandingInvoices = await tx.select().from(purchaseInvoices)
          .where(withTenant(purchaseInvoices, tenantId, and(
            ...
            or(eq(purchaseInvoices.status, 'Unpaid'), eq(purchaseInvoices.status, 'Partially Paid')),
        ...
        let remainingPaymentAmount = totalPaymentAmount;
        const distributedAmount = totalPaymentAmount;
        ...
        for (const invoice of outstandingInvoices) {
          ...
          const allocation = Math.min(remainingPaymentAmount, parseFloat(invoice.balanceAmount));
          ...
          let newStatus = newBalanceAmount <= 0.005 ? 'Paid' : newPaidAmount > 0 ? 'Partially Paid' : 'Unpaid';
        ...
        if (paymentsCreated.length) {
          const netPayment = distributedAmount - remainingPaymentAmount;
